...
using System;
using System.Text;
using AutoPilotControl.Nmea0183;
...
namespace Iot.Device.Nmea0183
{
    ...
    /// </remarks>
    [Serializable]
    public sealed class GeographicPosition : ICloneable, IFormattable
    {
        ...
        public static void GetDegreesMinutesSeconds(double angle, int secDigits, out double normalizedVal, out int degrees, out int minutes, out double seconds)
        {
            ...
            if (MissingMath.Round(seconds, secDigits) >= 60.0)
            {
                minutes += 1;
                seconds = 0;
        ...
        public static void GetDegreesMinutes(double angle, int minDigits, out double normalizedVal, out int degrees, out double minutes)
        {
            ...
            // If rounding the seconds to the given digit would print out "60",
            // add 1 to the minutes instead
            if (MissingMath.Round(minutes, minDigits) >= 60.0)
            {
                degrees += 1;
                minutes = 0;
                if (degrees >= 360)
        ...
        /// <param name="formatProvider">Culture info, ignored on Nanoframework</param>
            ...
            for (; formatIndex < format!.Length; formatIndex++)
            {
                ...
                switch (fmt)
                {
                    case 'D':
                        ...
                        if (element == 0)
                        {
                            output.Append(Latitude.ToString($"F{digits}") + "°");
                        }
                        else if (element == 1)
                        {
                            output.Append(Longitude.ToString($"F{digits}") + "°");
                        }
                        else if (element == 2)
                        {
                            output.Append(EllipsoidalHeight.ToString($"F{digits}"));
                        }
                    ...
                    case 'U':
                        ...
                        if (element == 0)
                        {
                            output.Append(Math.Abs(Latitude).ToString($"F{digits}") + "°");
                        }
                        else if (element == 1)
                        {
                            output.Append(Math.Abs(Longitude).ToString($"F{digits}") + "°");
                        }
                        else if (element == 2)
                        {
                    ...
                    case 'S':
                        ...
                        if (element == 0)
                        {
                            output.Append(GetLatitudeString(Latitude, digits, false));
                        }
                        else if (element == 1)
                        {
                            output.Append(GetLongitudeString(Longitude, digits, false));
                        }
                        else if (element == 2)
                        {
                            output.Append(EllipsoidalHeight.ToString($"F{digits}"));
                    ...
                    case 'M':
                        ...
                        string formatString =
                            $"{{0}}{DegreesSymbol} {{1:00.{new string('0', digits)}}}{MinutesSymbol}";

                        if (element == 0)
                        {
                            GetDegreesMinutes(Latitude, digits, out _, out degrees, out minutes);
                            output.Append(string.Format(formatString, degrees, minutes));
                        }
                        else if (element == 1)
                        {
                            GetDegreesMinutes(Longitude, digits, out _, out degrees, out minutes);
                            output.Append(string.Format(formatString, degrees, minutes));
                        }
                        else if (element == 2)
                        {
        ...
        private int GetNumDigits(string format, int index)
        {
            ...
            {
                if ((format[possibleNumberIndex] is >= '0' and <= '9'))
                {
                    // Must succeed
        ...
        {
            return (int)(Latitude * Longitude * EllipsoidalHeight);
        }
    }
}
